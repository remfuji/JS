...
function legalPawnMoves(chessboard, start) {
  ...
  //capire il contenuto della cella
  let color = chessboard[x][y][0] // prima lettera della stringa
  if (color == 'b') {
    ...
    return legalMoves
      ...
      // se il pedone e' alla posizione di partenza
      if (start[6] == 1 && isEmpty(chessboard, x, y + 2)) {
        legalMoves.push([start[0], start[1] - 1])
        legalMoves.push([start[0], start[1] - 2])
        return legalMoves
      }
...
function isOutOfTheChessboard(x, y) {
  return x < 0 || x > 7 || y < 0 || y > 7  //ritorna un booleano
}

//funzione dato un colore del turno prendo le mosse dello stesso 

function getPiecesByColor(chessboard, color) {
  //ciclare sulla chessboard 
  //mi chiedo se è dello stesso colore
  //push in array
  //return
  let pieces = []
  for (let i = 0; i < chessboard.length; i++) {
    let riga = chessboard[i];
...
function updateChessboard (chessboard, start, end) {
  let x = start[0]
  let y = start[1]
  let temp = chessboard[x][y]
  chessboard[end[0]][end[1]] = temp
  chessboard[x][y] = ""
  return chessboard
}

function getLegalMoves (chessboard, position) {
  //prendiamo il pezzo nella posizione 
  //in base al pezzo capiamo qual è la legal da invocare
  //una catena di if oppure come mappa 
  //invochiamo la legal ottenuta
  //ritorniamo array di mosse
  let map = {
    "T" : legalTowerMoves,
    "C" : legalKnightMoves,
    "A" : legalBishopMoves,
  ...
  let x = position[0]
  let y = position[1]
  let pezzo = chessboard[x][y] 
  let key = pezzo.substring(1)
  let legal = map[key]
  return legal(chessboard, position)
}
...
function printBetterChessboard(chessboard){
  ...
  function printRow(x,y){
    if(y < 0){
      return
    }

    let riga = ''
    for(let i = x; i < chessboard.length; i++){
      let temp = chessboard[i][y]
      let icon = chessIconsMap[temp]
      if(icon == undefined){
        riga += '  '
      } else {
        riga += ' ' + icon
